...
If you would rather wire up a project by hand, or you are adding napi-zig to a repo that already exists, this page is the complete path. It is a 1:1 of what `napi-zig new` writes, plus pointers into the publishing flow when you are ready to ship.
...
The version field starts at `0.0.0`. `napi-zig bump` manages it from here on.
...
That is the full development loop. Edit `src/lib.zig`, re-run `napi-zig build`, re-run your test.
...
- The **publish flow itself**: cross-compile, push to GitHub, run `napi-zig npm-init` once, then `napi-zig bump` for every release.

Walk through [Publishing to npm](/publishing) end to end. It assumes you reached it from this page or from `napi-zig new`; the steps are the same either way.
