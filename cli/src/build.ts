...
export async function buildRelease(optimize: string, options?: BuildReleaseOptions): Promise<void> {
  ...
  const destBase = join(process.cwd(), "bindings");
  ...
  if (existsSync(destBase) && onlyNames.length === 0) {
    ...
    if (orphans.length > 0) {
      blank();
      uiWarn(
        `Orphan director${orphans.length > 1 ? "ies" : "y"} in bindings/: ${orphans
          .map((n) => `bindings/${n}/`)
          .join(", ")}`,
      );
...
function detectExpectedTargets(): string[] {
  ...
  if (existsSync(bindingsDir)) {
    ...
    for (const addon of readdirSync(bindingsDir)) {
      const addonDir = join(bindingsDir, addon);
