// comptime module walker.
// takes a user's module type (@This()), finds all pub fns, and generates
// napi wrappers + the module init function.
...
fn AutoWrapper(comptime Module: type, comptime name: []const u8) type {
    ...
    return struct {
        ...
        fn extractString(env: Env, val: Val) !StringAlloc {
            const len = try val.stringLen(env);
...
fn exportNames(comptime Module: type) []const []const u8 {
    const decls = @typeInfo(Module).@"struct".decls;
    var names: []const []const u8 = &.{};
    for (decls) |decl| {
        if (decl.name[0] == '_') continue;
        if (@typeInfo(@TypeOf(@field(Module, decl.name))) != .@"fn") continue;
...
fn ModuleInit(comptime Module: type) type {
    const names = comptime exportNames(Module);
                ...
                if (comptime isRawFn(@TypeOf(@field(Module, name)))) {
                    const S = struct {
                        fn wrapper(raw_env: c.napi_env, info: c.napi_callback_info) callconv(.c) ?c.napi_value {
                            ...
                            const result = @field(Module, name)(e, @import("root.zig").CallInfo{ .raw = info }) catch |err| {
                    ...
                    try exports.setNamed(env, js_name, try env.function(js_name, S.wrapper));
                } else {
                    try exports.setNamed(env, js_name, try env.function(js_name, AutoWrapper(Module, name).call));
                }
            }
        }

        fn apiVersion() callconv(.c) u32 {
            return @import("root.zig").napi_module_version;
        }
    };
}
